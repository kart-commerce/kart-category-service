#!/usr/bin/env bash
# Usage:
#   scripts/migrate.sh                — apply pending migrations to CATEGORY_DB_CONNECTION_STRING
#   scripts/migrate.sh add <Name>     — generate a new migration from model changes (commit the result)
#
# (see CategoryDbContextFactory.cs for the same env var / localhost fallback)
set -euo pipefail
...
PROJECT_ARGS=(
  ...
  --startup-project src/Infrastructure/KartCategoryService.Infrastructure.csproj
)
