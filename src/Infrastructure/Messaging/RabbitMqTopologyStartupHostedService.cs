...
/// registered ahead of <see cref="OutboxRelayHostedService"/> (the generic host runs
/// <see cref="IHostedService.StartAsync"/> in registration order) so the topology exists
/// before anything publishes. A RabbitMQ outage at boot must not crash the process — same
/// degrade-not-crash contract as the relay — so failure here just logs; the relay
/// re-declares the same manifest idempotently inside its own retrying connection anyway.
/// </summary>
public sealed class RabbitMqTopologyStartupHostedService : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Could not declare RabbitMQ topology for {Service} at startup; the outbox relay " +
                "will retry this itself once RabbitMQ is reachable.",
                _manifest.Service);
        }
