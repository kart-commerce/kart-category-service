...
/// <summary>
/// Resolves RS256 signing keys for validating an Identity-issued access token (api-contract.yaml
/// clientCredentials scheme: "Identity-issued, never validated locally" - meaning Category checks
/// the signature/claims structurally, it never re-derives role/scope grants itself). Identity
/// exposes its public keys at GET /.well-known/jwks.json (kart-identity-service's JwksEndpoints) -
/// there is no full OIDC discovery document to point JwtBearerOptions.MetadataAddress at, so this
