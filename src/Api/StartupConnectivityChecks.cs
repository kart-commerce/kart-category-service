...
namespace KartCategoryService.Api;
...
public static class StartupConnectivityChecks
{
    public static async Task RunAsync(WebApplication app)
    {
        ...
        // swap out the real repository/cache/messaging registrations for in-memory fakes - real
    ...
    {
        logger.LogInformation("Connecting Category {Dependency} ...", dependency);
        try
        {
            await connect();
        }
        ...
        {
            logger.LogCritical(exception, "Failed to connect to Category {Dependency}", dependency);
            throw;
        }
