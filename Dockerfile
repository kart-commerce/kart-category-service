...
COPY KartCategoryService.sln Directory.Build.props nuget.config ./
...
RUN --mount=type=cache,target=/root/.nuget/packages,id=nuget-packages \
    dotnet restore src/Api/KartCategoryService.Api.csproj
...
RUN --mount=type=cache,target=/root/.nuget/packages,id=nuget-packages \
    dotnet publish src/Api/KartCategoryService.Api.csproj -c Release -o /app/publish --no-restore
